...
### 移动端虚拟键盘工具栏
- **快捷键支持**: ESC、Tab、Shift+Tab、Ctrl+C、Ctrl+D 等常用终端快捷键
- **导航键**: Home、End、Page Up、Page Down
- **方向键**: 完整的上下左右方向键布局
- **扩展功能**: Ctrl+L（清屏）、Ctrl+A/E（行首/行尾）、Ctrl+Z（挂起）
- **编辑器支持**: Ctrl+W/K/U/O/X 等 nano 编辑器快捷键
- **自动显隐**: 与 iOS 原生键盘同步显示/隐藏
- **安全区域**: 自动适配 iPhone 刘海屏安全区域
- **悬浮按钮**: 可点击切换工具栏显示/隐藏
- **响应式设计**: 仅在移动设备上显示，PC 端自动隐藏
