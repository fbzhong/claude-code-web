...
export default async function sessionsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: SessionParams }>('/sessions/:sessionId', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    try {
      const user = (request as any).user;
  ...
  fastify.post<{ Body: AttachSessionRequest }>('/sessions/attach', {
    preHandler: [fastify.authenticate],
    schema: {
      body: {
        type: 'object',
    ...
    try {
      ...
      const session = await sessionManager.attachToSession(sessionId, user.id);
  ...
  // Delete session
  fastify.delete<{ Params: SessionParams }>('/sessions/:sessionId', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    try {
      const user = (request as any).user;
