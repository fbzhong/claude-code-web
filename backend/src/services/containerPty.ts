...
export class ContainerPty extends EventEmitter implements Partial<IPty> {
  ...
  constructor(
    ...
    console.log(
      `[ContainerPty] Creating PTY for container ${containerId} with command:`,
      command
    );
  ...
  private start(): void {
    console.log(
      `[ContainerPty] Starting interactive process in container ${this.containerId}`
    );
    ...
    try {
      ...
      console.log(
        `[ContainerPty] Started process for container ${this.containerId}`
      );
    } catch (error) {
    ...
    this.containerProcess.stdout.on("data", (data: Buffer) => {
      // console.log(
      //   `[ContainerPty] stdout data from container ${this.containerId}:`,
      //   data.toString()
      // );
    ...
    this.containerProcess.stderr.on("data", (data: Buffer) => {
      console.log(
        `[ContainerPty] stderr data from container ${this.containerId}:`,
        data.toString()
      );
      this.emit("data", data.toString());
    });
    ...
    console.log(
      `[ContainerPty] stdin writable: ${this.containerProcess.stdin.writable}`
    );
    ...
    this.containerProcess.on("exit", (code: number, signal: string) => {
      console.log(
        `[ContainerPty] Process exited for container ${this.containerId}: code=${code}, signal=${signal}`
      );
    ...
    this.containerProcess.on("error", (error: Error) => {
      console.error(
        `[ContainerPty] Process error for container ${this.containerId}:`,
        error
      );
  ...
  write(data: string): void {
    ...
    if (!this.killed && this.containerProcess && this.containerProcess.stdin) {
      ...
      console.warn(
        `[ContainerPty] Cannot write - killed: ${this.killed}, process: ${!!this
          .containerProcess}, stdin: ${!!this.containerProcess?.stdin}`
      );
    }
  }
  ...
  resize(cols: number, rows: number): void {
    ...
    this.containerManager
      .exec(this.containerId, ["sh", "-c", `stty cols ${cols} rows ${rows}`], {
        user: this.options.user,
      })
      .catch(() => {
  ...
  static spawn(
    ...
    return new ContainerPty(containerId, containerManager, command, {
