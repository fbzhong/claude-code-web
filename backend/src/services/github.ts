...
export class GitHubService {
  ...
  async getUserRepos(accessToken: string, page = 1, perPage = 100): Promise<GitHubRepo[]> {
    try {
  ...
  async saveConnection(userId: string, tokenData: GitHubTokenResponse): Promise<void> {
    ...
    await pool.query(
      ...
      [userId, tokenData.access_token, tokenData.refresh_token, tokenData.token_type, tokenData.scope, expiresAt]
    );
  }
  ...
  async deleteConnection(userId: string): Promise<void> {
    ...
    if (connection) {
      try {
        ...
        this.fastify.log.warn('Failed to revoke token on GitHub, continuing with local deletion');
      }
    }
  ...
  async syncUserRepos(userId: string, accessToken: string): Promise<void> {
    ...
    for (const repo of repos) {
      await pool.query(
        ...
        [userId, connectionId, repo.id, repo.name, repo.full_name, repo.owner.login, repo.private, repo.clone_url, repo.ssh_url]
      );
    }
  }
  ...
  async checkAndRefreshToken(userId: string): Promise<string> {
    ...
    if (connection.expires_at && connection.refresh_token) {
      ...
      if (expiresAt < oneHourFromNow) {
        try {
          const newTokenData = await this.refreshToken(connection.refresh_token);
          await this.saveConnection(userId, newTokenData);
          return newTokenData.access_token;
        } catch (error) {
          this.fastify.log.warn('Failed to refresh token, using existing token');
        }
      }
    }
